...
<script src="{{ asset('/js/bootstrap-multiselect.js') }}"></script>
    ...
    <div class="row justify-content-center" style="margin-top:10%;">
        <a  href="{{ route('view.user',['edit' => false ]) }}"><button type="submit" class="btn btn-warning" style="float:left;">Back</button></a>
        <div class="col-md-8">
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('edit.user') }}">
                        ...
                        <div class="form-row">
                            <div class="form-group col-md-6">
                                ...
                                <input type="text" name="nick" class="form-control" id="inputNick" value=" {{{ isset($user['nick']) ? $user['nick'] : ''  }}}"
                                       placeholder=" {{{ isset($user['nick']) ? $user['nick'] : ''  }}}">
                            </div>
                            <div class="form-group col-md-6">
                                ...
                                <input type="text" name="login" class="form-control" id="inputlogin" value=" {{{ isset($user['login']) ? $user['login'] : ''  }}}"
                                       placeholder=" {{{ isset($user['login']) ? $user['login'] : ''  }}}">
                            </div>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="password" name="password2" class="form-control" id="inputPass2" placeholder="confirm password">
                        </div>
                        <div class="form-row">
                            <div class="form-group col-md-6">
                               ...
                               @foreach ($all_interests as $interest)
                                    @if(in_array($interest,$interests))
                                         <option value="{{ $interest->idInterests }}" selected >{{ $interest->interest_name }}</option>
                                    @else
                            ...
                            <div class="form-group col-md-4"  id="grup">
                                ...
                                <select name="groups[]" id="multi-groups" multiple="multiple">
                                    @foreach ($groups as $group)
                                        <option value="{{ $group->idGroups }}" selected >{{ $group->group_name }}</option>
                                    @endforeach
                                </select>
                            </div>
